...
class wpdb2 {
		...
		if ( $this->has_cap( 'collation' ) && ! empty( $charset ) ) {
			...
			if ( $this->use_mysqli ) {
				...
				if ( function_exists( 'wpsql_set_charset' ) && $this->has_cap( 'set_charset' ) ) {
					$set_charset_succeeded = wpsql_set_charset( $charset, $dbh );
				}
				if ( $set_charset_succeeded ) {
					$query = $this->prepare( 'SET NAMES %s', $charset );
					if ( ! empty( $collate ) ) {
	...
	function _real_escape( $string ) {
		if ( $this->dbh ) {
			if ( $this->use_mysqli ) {
				...
				$escaped = wpsql_real_escape_string( $string, $this->dbh );
			}
		} else {
		...
		if ( $this->use_mysqli ) {
			...
			if ( $this->dbh->connect_errno ) {
				...
				if ( $this->has_connected ) {
					...
					$attempt_fallback = false;
				} elseif ( ! function_exists( 'wpsql_connect' ) ) {
					$attempt_fallback = false;
				}
	 ...
	 * socket like wpsql_connect does. This duplicates how wpsql_connect detects
	...
	public function check_connection( $allow_bail = true ) {
		if ( $this->use_mysqli ) {
			...
			if ( ! empty( $this->dbh ) && wpsql_ping( $this->dbh ) ) {
				return true;
			}
		}
				...
				if ( ! empty( $value['db'] ) ) {
					...
					if ( $this->charset ) {
						...
						if ( $this->use_mysqli ) {
							...
							$connection_charset = wpsql_client_encoding();
						}
					}
	...
	protected function load_col_info() {
		...
		if ( $this->use_mysqli ) {
			...
			$num_fields = wpsql_num_fields( $this->result );
			for ( $i = 0; $i < $num_fields; $i++ ) {
	 ...
	 *
	 * @global string $wp_version             The WordPress version string.
	 * @global string $required_wpsql_version The required MySQL version string.
	 *
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'utf8mb4':      // @since 4.1.0
				...
				if ( $this->use_mysqli ) {
					...
					$client_version = wpsql_get_client_info();
				}
